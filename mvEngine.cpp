...
void mv::Engine::recreate_swapchain(void)
{
    ...
    if (!mv_device)
        throw std::runtime_error("mv device handler is somehow null, tried to recreate swap chain :: main");
...
void mv::Engine::go(void)
{
    ...
    descriptor_allocator = std::make_shared<mv::Allocator>(std::weak_ptr<mv::Device>(mv_device));
    ...
    std::cout << "[+] xcb conn smart ptr => " << xcb_conn.get() << " :: points to => " << *xcb_conn << "\n";
...
void mv::Engine::prepare_pipeline(void)
{
    ...
    cba_state.colorWriteMask = vk::ColorComponentFlagBits::eR |
...
void mv::Engine::record_command_buffer(uint32_t image_index)
{
    ...
    for (auto &model : *collection_handler->models)
    {
        ...
        {
            command_buffers->at(image_index).bindPipeline(vk::PipelineBindPoint::eGraphics, pipelines->at("no_sampler"));
        }
        for (auto &object : *model.objects)
        {
            for (auto &mesh : *model._meshes)
            {
                ...
                if (model.has_texture)
                {
                    ...
                    command_buffers->at(image_index).bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline_layouts->at("sampler"), 0, to_bind, nullptr);
                }
                else
                {
                    command_buffers->at(image_index).bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline_layouts->at("no_sampler"), 0, to_bind, nullptr);
                }
                // Bind mesh buffers
...
void mv::Engine::draw(size_t &cur_frame, uint32_t &cur_index)
{
    ...
    vk::Result result = mv_device->logical_device->acquireNextImageKHR(*swapchain->swapchain, UINT64_MAX, semaphores->present_complete, nullptr, &cur_index);
    ...
    default: // unhandled error occurred
        throw std::runtime_error("Unhandled error case while acquiring a swapchain image for rendering");
        break;
    }
