...
void mv::Engine::go(void) {
  ...
  */
  ...
  mouse->delta_style = mouse::delta_calc_style::from_last;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    while (accumulated >= timestep) {
      ...
      // Get input events
      mv::keyboard::event kbd_event = read_key();
      ...
      if (camera->type == Camera::camera_type::third_person) {
        ...
        */
        ...
        if (mouse->is_dragging) {
          ...
          if (mouse->drag_delta_x > 0) {
            camera->adjust_orbit(-fabs(mouse->drag_delta_x * 0.4f), mouse->stored_orbit);
          ...
          if (mouse->drag_delta_y > 0) {
            camera->adjust_pitch(fabs(mouse->drag_delta_y * 0.1f), mouse->stored_pitch);
        ...
        */
        ...
        if (kbd_event.type == mv::keyboard::event::press &&
            kbd_event.code == GLFW_KEY_LEFT_CONTROL) {
          std::cout << "Mouse data\n";
        ...
        */
        ...
        // forward only
        if (is_keystate(GLFW_KEY_W) && !is_keystate(GLFW_KEY_S) && !is_keystate(GLFW_KEY_A) &&
            !is_keystate(GLFW_KEY_D)) {
        ...
        if (is_keystate(GLFW_KEY_SPACE) && added == false) {
          // added = true;
          int min = 0;
          int max = 90;
          int z_max = 60;
  ...
  */
  for (const auto &job : to_render_map) {
    ...
    if (job.first == "reticle_raycast" && job.second) {
      ...
      command_buffers->at(image_index)
          .bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipeline_layouts->at("no_sampler"),
                              0, to_bind, nullptr);
...
inline void mv::Engine::go_setup(void) {
  ...
  */
  ...
  descriptor_allocator->update_set(*mv_device, reticle_obj.uniform_buffer.descriptor,
                                   reticle_obj.model_descriptor, 0);
...
void mv::key_callback(GLFWwindow *window, int key, [[maybe_unused]] int scancode, int action,
  ...
  if (action == GLFW_PRESS) {
    mv::Engine::on_key_press(key);
  } else if (action == GLFW_RELEASE) {
    mv::Engine::on_key_release(key);
  }
  return;
}
...
/*
  KEYBOARD METHODS
*/
mv::keyboard::event mv::Engine::read_key(void) {
...
void mv::Engine::on_key_press(int code) {
  key_states[code] = true;
  key_buffer.push(keyboard::event(keyboard::event::etype::press, code));
  trim_buffer(key_buffer, mv::max_buffer_size);
  return;
}

void mv::Engine::on_key_release(int code) {
...
void mv::Engine::clear_state(void) {
  key_states.reset();
  return;
}

bool mv::Engine::is_keystate(int code) {
  // sanity check
  if (code < 0) {
    return false;
  }
  return mv::key_states[code];
}

bool mv::Engine::is_key(GLFWwindow *window, int code) {
  // sanity check
  if (code < 0) {
    return false;
  }
