...
void
Camera::adjustMovement (glm::vec3 p_Delta)
{
  getFrontFace ();
...
void
Camera::adjustZoom (float p_Delta)
{
  zoomAccel += p_Delta;
  return;
}
...
void
Camera::adjustPitch (float p_Delta, float p_StartPitch)
{
...
void
Camera::adjustPitch (float &p_Angle)
{
  if (p_Angle > 89.9f)
    {
      p_Angle = 89.9f;
...
void
Camera::adjustOrbit (float p_Delta, float p_StartOrbit)
{
...
void
Camera::adjustOrbit (float p_Delta)
{
  orbitAngle += p_Delta;
  return;
}
...
void
Camera::lerpOrbit (float p_Delta)
{
  targetOrbit += p_Delta;
...
void
Camera::lerpPitch (float p_Delta)
{
  targetPitch += p_Delta;
...
void
Camera::realignOrbit (float &p_Angle)
{
  if (p_Angle > 359.9f)
    {
...
bool
Camera::checkZoom (float p_Delta)
{
...
void
Camera::getFrontFace (float p_OrbitAngle)
{
  glm::vec3 fr;
...
void
Camera::move (float p_Angle, glm::vec3 p_TargetAxii, bool p_Boost)
{
  constexpr float newMoveSpeed = MOVESPEED * 30.0f;
...
void
Camera::moveDown (void)
{
  position.y += MOVESPEED * 1.5f;
  return;
}
void
Camera::moveDown (bool p_IsBoost)
{
  if (p_IsBoost)
    position.y += MOVESPEED * 5.0f;
  else
...
bool
Camera::setCameraType (CameraType p_CameraType, glm::vec3 p_Position)
{
  switch (p_CameraType)
    {
        ...
        if (!target)
          {
            ptrLogger->logMessage (
                eError, "Can't set to third person due to no target");
            return false;
          }
        break;
      }
...
glm::vec3
Camera::rotateVector (float p_OrbitAngle, glm::vec4 p_TargetAxii)
{
