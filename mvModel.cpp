...
void Model::load(Engine *p_Engine, Allocator &p_DescriptorAllocator, const char *p_Filename, bool p_OutputDebug)
{
    ...
    const aiScene *aiScene =
        importer.ReadFile(p_Filename, aiProcess_JoinIdenticalVertices | aiProcess_FlipUVs | aiProcess_Triangulate |
    ...
    for (auto &mesh : *loadedMeshes)
    {
        ...
        if (mesh.mtlIndex >= 0)
        {
            ...
            p_DescriptorAllocator.allocateSet(samplerLayout, textureDescriptors.at(mesh.mtlIndex).first);

            p_DescriptorAllocator.updateSet(textureDescriptors.at(mesh.mtlIndex).second.mvImage.descriptor,
...
struct Mesh Model::processMesh(Engine *p_Engine, aiMesh *p_Mesh, const aiScene *p_Scene)
{
    ...
    std::vector<Texture> diffuseMaps =
        loadMaterialTextures(p_Engine, material, aiTextureType_DIFFUSE, "texture_diffuse", p_Scene, m.mtlIndex);
    texs.insert(texs.end(), std::make_move_iterator(diffuseMaps.begin()), std::make_move_iterator(diffuseMaps.end()));
...
std::vector<Texture> Model::loadMaterialTextures(Engine *p_Engine, aiMaterial *p_Material, aiTextureType p_Type,
...
std::ostream &operator<<(std::ostream &p_OutputStream, const Vertex &p_Vertex)
{
    p_OutputStream << p_Vertex.position.x << " " << p_Vertex.position.y << " " << p_Vertex.position.z << " "
                   ...
                   << p_Vertex.color.b << " " << p_Vertex.color.a << "|" << p_Vertex.uv.x << " " << p_Vertex.uv.y << " "
                   << 0.0f << " " << 0.0f << "\n";
