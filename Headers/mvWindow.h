...
class MWindow {
 public:
  ...
  MWindow(int w, int h, std::string title) {
    ...
    if (!tmp) throw std::runtime_error("Failed to connect to x server");
    ...
    // get screen
    xcb_screen_t *screen =
        xcb_setup_roots_iterator(xcb_get_setup(*xcb_conn)).data;
    if (!screen) throw std::runtime_error("xcb failed to get screen");
    ...
    xcb_create_window(*xcb_conn, XCB_COPY_FROM_PARENT, *xcb_win, screen->root,
    ...
    xcb_intern_atom_reply_t *protocols_reply =
        xcb_intern_atom_reply(*xcb_conn, protocols_cookie, 0);
    ...
    xcb_change_property(*xcb_conn, XCB_PROP_MODE_REPLACE, *xcb_win,
    ...
    // xcb_setup_roots_iterator(xcb_get_setup(xcb_conn.get())); for (;
    // scr_iter.rem; xcb_scr_num--, xcb_screen_next(&scr_iter))
    // {
    //     if (xcb_scr_num <= 0)
    //     {
    ...
    // fetch xcb key syms
    xcb_key_syms = xcb_key_symbols_alloc(*xcb_conn);

    if (!xcb_key_syms)
      throw std::runtime_error(
          "Failed to get key syms, program initialization :: main");

    // initialize keyboard handler
    kbd = std::make_unique<mv::keyboard>(
        std::weak_ptr<xcb_connection_t *>(xcb_conn), xcb_key_syms);

    // initialize mouse handler
    std::cout << "[+] initializing mouse handler\n";
    mouse =
        std::make_unique<mv::mouse>(std::weak_ptr<xcb_connection_t *>(xcb_conn),
  ...
  ~MWindow() {
    ...
    if (semaphores) {
      if (semaphores->present_complete)
        mv_device->logical_device->destroySemaphore(
            semaphores->present_complete, nullptr);
      if (semaphores->render_complete)
        mv_device->logical_device->destroySemaphore(semaphores->render_complete,
                                                    nullptr);
      semaphores.reset();
    }
    ...
    if (depth_stencil) {
      ...
      if (depth_stencil->view) {
        mv_device->logical_device->destroyImageView(depth_stencil->view,
                                                    nullptr);
      }
    ...
    // free symbol map
    xcb_key_symbols_free(xcb_key_syms);

    if (xcb_win && xcb_conn) xcb_destroy_window(*xcb_conn, *xcb_win);
    if (xcb_conn) xcb_disconnect(*xcb_conn);
    xcb_win.reset();
    xcb_conn.reset();
    return;
  }  // end destructor
  ...
  // based on xcb events
  bool handle_x_event(xcb_generic_event_t *event);
 ...
 public:
  ...
  Timer fps;
  bool good_init = true;
  vk::ClearColorValue default_clear_color =
 ...
 protected:
  ...
  xcb_key_symbols_t *xcb_key_syms = nullptr;
