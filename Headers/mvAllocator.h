...
namespace mv {
  struct Allocator {
    ...
    void create_layout(const mv::Device &m_dvc, std::string layout_name, vk::DescriptorType type,
                       uint32_t count, vk::ShaderStageFlagBits shader_stage_flag_bits,
                       uint32_t binding) {
    ...
    vk::DescriptorSetLayout get_layout(std::string layout_name) {
      if (!layouts)
        throw std::runtime_error("Requested layout but container never initialized => " +
                                 layout_name);
    ...
    void allocate_set(const mv::Device &m_dvc, vk::DescriptorSetLayout &layout,
    ...
    void allocate_set(const mv::Device &m_dvc, Container *container,
    ...
    void update_set(const mv::Device &m_dvc, vk::DescriptorBufferInfo &buffer_info,
                    vk::DescriptorSet &dst_set, uint32_t dst_binding) {
      Container *container = &containers->at(current_pool);

      if (!container)
        throw std::runtime_error(
            "Failed to get current pool handle, updating set :: descriptor handler");
    ...
    void update_set(const mv::Device &m_dvc, Container *container,
                    vk::DescriptorBufferInfo &buffer_info, vk::DescriptorSet &dst_set,
    ...
    void update_set(const mv::Device &m_dvc, Container *container,
                    vk::DescriptorImageInfo &image_info, vk::DescriptorSet &dst_set,
                    uint32_t dst_binding) {
      vk::WriteDescriptorSet update_info;
      update_info.dstBinding = dst_binding;
      update_info.dstSet = dst_set;
    ...
    struct Container {
      enum Status { Clear, Usable, Fragmented, Full };
