...
class GuiHandler
{
    ...
    */
    ...
    void (*showOpenDialog)(GuiHandler *p_This) = [](GuiHandler *p_This) {
                                   ...
                                   p_This->fileMenu.openModal.flags))
        {
            ...
            // Red for directory
            // White for file
            p_This->outputSelectableList(filesInDirectory, p_This->fileSystem.currentDirectory, ListType::eMapFiles);
  ...
  private:
    ...
    inline void renderDebugModal(std::vector<std::pair<LogHandler::MessagePriority, std::string>> p_DebugMessageList)
    {
        // Terrain info
        auto viewport = ImGui::GetMainViewport();
        auto pos = viewport->Size;
