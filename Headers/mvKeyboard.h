...
struct keyboard {
  ...
  keyboard(std::weak_ptr<xcb_connection_t *> xcb_conn,
    ...
    if (!test)
      throw std::runtime_error(
          "invalid xcb connection handle passed to keyboard handler, "
          "initialization :: keyboard handler");
    if (!*test)
      throw std::runtime_error(
          "invalid xcb connection handle passed to keyboard handler, "
          "initialization :: keyboard handler");
  ...
  struct event {
    ...
    inline bool is_press(void) const noexcept { return type == etype::press; }
    inline bool is_release(void) const noexcept {
      return type == etype::release;
    }
    inline bool is_valid(void) const noexcept { return type != etype::invalid; }
  };

 public:
  inline bool is_keypress(mv::keyboard::key mv_key_code) {
    if (static_cast<unsigned int>(mv_key_code) < 0 ||
        static_cast<unsigned int>(mv_key_code) > mv::keyboard::max_key_count) {
      return false;
    }
  ...
  inline mv::keyboard::key keysym_to_mvkey(
      xcb_keysym_t xcb_key_sym) const noexcept {
    // sanity check
    if (xcb_key_sym == 0) {
  ...
  inline mv::keyboard::key keycode_to_mvkey(
      xcb_keycode_t xcb_key_code) const noexcept {
    // sanity check
    if (xcb_key_code == 0) {
      return keyboard::key::invalid;
    }
    ...
    xcb_keysym_t key =
        xcb_key_symbols_get_keysym(xcb_key_syms, xcb_key_code, 0);

    return keysym_to_mvkey(key);
  }
  ...
  inline void on_key_press(mv::keyboard::key mv_key_code) {
    ...
    key_buffer.push(
        keyboard::event(keyboard::event::etype::press, mv_key_code));
  ...
  inline void on_key_release(mv::keyboard::key mv_key_code) {
    ...
    key_buffer.push(
        keyboard::event(keyboard::event::etype::release, mv_key_code));
    trim_buffer(key_buffer, mv::keyboard::max_buffer_size);
    return;
  }
 ...
 private:
  ...
  // event queue for keyboard related events retreived from x server
