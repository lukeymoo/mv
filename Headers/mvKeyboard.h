...
namespace mv {
  ...
  public:
    ...
    inline mv::keyboard::key keysym_to_mvkey(KeySym x11_keysym) const noexcept {
      // sanity check
      if (x11_keysym == 0) {
    ...
    mv::keyboard::key keycode_to_mvkey(Display *display, const KeyCode x11_keycode) const {
      // sanity check
      if (x11_keycode == 0) {
        return keyboard::key::invalid;
      }

      // auto key = XKeycodeToKeysym(display, x11_keycode, 0);
      auto key = XkbKeycodeToKeysym(display, x11_keycode, 0, 0);
