...
namespace mv {
  extern std::bitset<350> key_states;
  extern std::queue<mv::keyboard::event> key_buffer;
    ...
    inline void end_command_buffer(vk::CommandBuffer buffer) {
      ...
      if (!mv_device)
        throw std::runtime_error(
            "attempted to end command buffer recording but mv device handler is nullptr");

      if (!mv_device->command_pool)
        throw std::runtime_error(
            "attempted to end command buffer recording but command pool is nullptr");

      if (!mv_device->graphics_queue)
        throw std::runtime_error(
            "attempted to end command buffer recording but graphics queue is nullptr");
  ...
  public:
    mv::keyboard::event read_key(void);

    static void on_key_press(int code);
    static void on_key_release(int code);
    static void clear_state(void);

    static inline bool is_keystate(int code);

    static inline bool is_key(GLFWwindow *window, int code);
  };
  ...
  static inline void raycast_p1(const mv::Engine &engine, glm::vec3 pos) {
    ...
    void *reticle_mapped = nullptr;
    reticle_mapped = engine.mv_device->logical_device->mapMemory(engine.reticle_mesh.vertex_memory,
                                                                 0, sizeof(Vertex));
    if (!reticle_mapped)
