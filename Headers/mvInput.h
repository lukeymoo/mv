...
public:
  struct Event
  {
    ...
    inline bool
    isPress (void) const noexcept
    {
      return type == Type::ePress;
    }
    inline bool
    isRelease (void) const noexcept
    {
  ...
  std::queue<struct Keyboard::Event> keyBuffer;

  template <typename T>
  void trimBuffer (std::queue<T> &p_Buffer, uint32_t p_MaxSize);

  struct Keyboard::Event read (void);
...
public:
  struct Event
  {
    ...
    // Event(Type p_EventType, int p_CursorX, int p_CursorY, bool
    // p_LeftButtonStatus, bool p_MiddleButtonStatus,
  ...
  const uint32_t maxBufferSize = 24;
  std::queue<struct Mouse::Event> mouseBuffer;
