...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_HasDisplayRect = 1
                                          << 1, // LastItemDisplayRect is valid
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImVec2 WorkOffsetMax; // Work Area: Offset from Pos+Size to bottom-right
                          ...
                          // (0,-status_bar_height).
    ImVec2 CurrWorkOffsetMin; // Work Area: Offset being built/increased during
                              // current frame
    ImVec2 CurrWorkOffsetMax; // Work Area: Offset being built/decreased during
                              // current frame
    ...
    void UpdateWorkRect()
    {
        WorkPos = ImVec2(Pos.x + WorkOffsetMin.x, Pos.y + WorkOffsetMin.y);
        WorkSize =
            ImVec2(ImMax(0.0f, Size.x - WorkOffsetMin.x + WorkOffsetMax.x),
                   ImMax(0.0f, Size.y - WorkOffsetMin.y + WorkOffsetMax.y));
    }
};
...
struct ImGuiContext
{
    ...
    bool HoveredIdUsingMouseWheel; // Hovered widget will use mouse wheel.
    ...
    bool
        ActiveIdUsingMouseWheel; // Active widget will want to read mouse wheel.
                                 // Blocks scrolling the underlying window.
    ...
    int TabFocusRequestCurrCounterRegular;  // Any item being requested for
                                            // focus, stored as an index (we on
                                            // layout to be stable between the
                                            // frame pressing TAB and the next
                                            // frame, semi-ouch)
    int TabFocusRequestCurrCounterTabStop;  // Tab item being requested for
    ...
    float TooltipSlowDelay; // Time before slow tooltips appears (FIXME: This is
                            // temporary until we merge in tooltip
                            // timer+priority work)
    ImVector<char>
    ...
    ImGuiContext(ImFontAtlas *shared_font_atlas)
    {
        ...
        HoveredIdUsingMouseWheel = HoveredIdPreviousFrameUsingMouseWheel =
            false;
        ...
        TabFocusRequestCurrCounterRegular = TabFocusRequestCurrCounterTabStop =
            INT_MAX;
        TabFocusRequestNextCounterRegular = TabFocusRequestNextCounterTabStop =
            INT_MAX;
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    int NavLayerActiveMask; // Which layers have been written to (result from
                            // previous frame)
...
struct ImGuiTableColumn
{
    ...
    bool IsEnabled; // Is the column not marked Hidden by the user? (even if off
    ...
    ImU8 SortDirectionsAvailMask : 4;  // Mask of available sort directions
                                       // (1-bit each)
    ImU8 SortDirectionsAvailList;      // Ordered of available sort directions
                                       // (2-bits each)
...
// FIXME-TABLE: transient data could be stored in a per-stacked table structure:
// DrawSplitter, SortSpecs, incoming RowData
struct ImGuiTable
{
    ImGuiID ID;
    ...
    ImDrawListSplitter
        DrawSplitter; // We carry our own ImDrawList splitter to allow recursion
                      // (FIXME: could be stored outside, worst case we need 1
                      // splitter per recursing table)
    ImGuiTableColumnSortSpecs SortSpecsSingle;
    ImVector<ImGuiTableColumnSortSpecs>
        SortSpecsMulti; // FIXME-OPT: Using a small-vector pattern would work be
                        // good.
...
namespace ImGui
{
    ...
    // - You are calling ImGui functions after ImGui::EndFrame()/ImGui::Render()
    // and before the next ImGui::NewFrame(), which is also illegal.
    ...
    IMGUI_API void Shutdown(
        ImGuiContext *context); // Since 1.60 this is a _private_ function. You
    ...
    IMGUI_API ImVec2 CalcItemSize(ImVec2 size, float default_w,
                                  float default_h);
    IMGUI_API float CalcWrapWidthForPos(const ImVec2 &pos, float wrap_pos_x);
    IMGUI_API void PushMultiItemsWidths(int components, float width_full);
    IMGUI_API void PushItemFlag(ImGuiItemFlags option, bool enabled);
    IMGUI_API void PopItemFlag();
    IMGUI_API bool IsItemToggledSelection(); // Was the last item selection
                                             // toggled? (after Selectable(),
                                             // TreeNode() etc. We only returns
                                             // toggle _event_ in order to
    ...
    IMGUI_API int CalcTypematicRepeatAmount(float t0, float t1,
                                            float repeat_delay,
                                            float repeat_rate);
    IMGUI_API void ActivateItem(
        ImGuiID id); // Remotely activate a button, checkbox, tree node etc.
    ...
    // Focus Scope (WIP)
    // This is generally used to identify a selection set (multiple of which may
    // be in the same window), as selection patterns generally need to react
    // (e.g. clear selection) when landing on an item of the set.
    ...
    IMGUI_API int TableGetHoveredColumn(); // May use (TableGetColumnFlags() &
                                           ...
                                           // instead. Return hovered column.
                                           // return -1 when table is not
                                           // hovered. return columns_count if
                                           // the unused space at the right of
    ...
    IMGUI_API const char *TableGetColumnName(const ImGuiTable *table,
                                             int column_n);
    IMGUI_API ImGuiID TableGetColumnResizeID(const ImGuiTable *table,
                                             int column_n, int instance_no = 0);
    IMGUI_API float TableGetMaxColumnWidth(const ImGuiTable *table,
                                           int column_n);
    IMGUI_API void TableSetColumnWidthAutoSingle(ImGuiTable *table,
                                                 int column_n);
    ...
    IMGUI_API bool TabItemEx(ImGuiTabBar *tab_bar, const char *label,
    ...
    // Render helpers
    // AVOID USING OUTSIDE OF IMGUI.CPP! NOT FOR PUBLIC CONSUMPTION. THOSE
    // FUNCTIONS ARE A MESS. THEIR SIGNATURE AND BEHAVIOR WILL CHANGE, THEY NEED
    // TO BE REFACTORED INTO SOMETHING DECENT. NB: All position are in absolute
    // pixels coordinates (we are never using window coordinates internally)
    ...
    IMGUI_API void RenderCheckMark(ImDrawList *draw_list, ImVec2 pos, ImU32 col,
                                   float sz);
    IMGUI_API void RenderMouseCursor(ImDrawList *draw_list, ImVec2 pos,
    ...
    IMGUI_API ImRect GetWindowScrollbarRect(ImGuiWindow *window,
                                            ImGuiAxis axis);
    ...
    IMGUI_API bool SliderBehavior(const ImRect &bb, ImGuiID id,
                                  ...
                                  ImRect *out_grab_bb);
    IMGUI_API bool SplitterBehavior(const ImRect &bb, ImGuiID id,
                                    ImGuiAxis axis, float *size1, float *size2,
    ...
    IMGUI_API bool TreeNodeBehaviorIsOpen(
        ImGuiID id, ImGuiTreeNodeFlags flags =
                        0); // Consume previous SetNextItemOpen() data, if any.
                            // May return true when logging
    IMGUI_API void TreePushOverrideID(ImGuiID id);

    // Template functions are instantiated in imgui_widgets.cpp for a finite
    // number of types. To use them externally (for custom widget) you may need
    // an "extern template" statement in your code in order to link to existing
    // instances and silence Clang warnings (see #2036). e.g. " extern template
    // IMGUI_API float RoundScalarWithFormatT<float, float>(const char* format,
    ...
    IMGUI_API bool SliderBehaviorT(const ImRect &bb, ImGuiID id,
                                   ImGuiDataType data_type, T *v, T v_min,
                                   T v_max, const char *format,
    ...
    IMGUI_API void DataTypeApplyOp(ImGuiDataType data_type, int op,
                                   void *output, const void *arg_1,
                                   const void *arg_2);
    ...
    IMGUI_API bool InputTextEx(const char *label, const char *hint, char *buf,
                               ...
                               void *user_data = NULL);
    IMGUI_API bool TempInputText(const ImRect &bb, ImGuiID id,
                                 const char *label, char *buf, int buf_size,
                                 ImGuiInputTextFlags flags);
    IMGUI_API bool TempInputScalar(const ImRect &bb, ImGuiID id,
