                           ...
                           bool *out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
        ...
        if (IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
        {
            hovered = true;
            SetHoveredID(id);
            if (CalcTypematicRepeatAmount(g.HoveredIdTimer + 0.0001f -
                                              g.IO.DeltaTime,
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos)
{
    ...
    // isn't the regular behavior of buttons, but it doesn't affect the user
                       ...
                       ImGuiComboFlags flags)
{
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(preview_pos, ImVec2(value_x2, frame_bb.Max.y),
    ...
    // Position the window given a custom constraint (peak into expected window
    // size so we can position it) This might be easier to express with an
    // hypothetical SetNextWindowPosConstraints() function.
                       ...
                       const char *format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        // Experimental: simple click (without moving) turns Drag into an
        // InputText
        // FIXME: Currently polling ImGuiConfigFlags_IsTouchScreen, may either
        // poll an hypothetical ImGuiBackendFlags_HasKeyboard and/or an explicit
        // drag settings.
                                 ...
                                 float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        // "mathematically correct" but non-intuitive behaviors like a
        // fully-left slider not actually reaching the minimum value
        if (t <= 0.0f)
            result = v_min;
        else if (t >= 1.0f)
            result = v_max;
                ...
                0.0f) // Range crosses zero, so we have to do this in two parts
            {
                ...
                if (t_with_flip >= zero_point_snap_L &&
                    ...
                    result = (TYPE)0.0f; // Special case to make getting exactly
                                         // zero possible (the epsilon prevents
                                         // it otherwise)
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf),
            ...
            value_changed =
                DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data,
                                        data_type, p_data, format);
...
#include "imstb_textedit.h"
    ...
    // creates two separate records, so we perform it manually. (FIXME: Report
    // to nothings/stb?)
                                     ...
                                     void *user_data)
{
         ...
         ImGuiInputTextFlags_CharsScientific))
    {
        ...
        // "de_DE.UTF-8");' which affect the output/input of printf/scanf. The
        // standard mandate that programs starts in the "C" locale where the
        // decimal point is '.'. We don't really intend to provide widespread
        // support for it, but out of empathy for people stuck with using odd
        // API, we support the bare minimum aka overriding the decimal point.
                        ...
                        void *callback_user_data)
{
    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        if (flags &
            (ImGuiInputTextFlags_CallbackCompletion |
             ImGuiInputTextFlags_AllowTabInput)) // Disable keyboard tabbing out
                                                 // as we will use the \t
                                                 // character.
    ...
    if (g.ActiveId == id)
    {
        ...
        // It is ill-defined whether the backend needs to send a \t character
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < state->ScrollX)
                    ...
                    state->ScrollX = IM_FLOOR(cursor_offset.x - visible_width +
                                              scroll_increment_x);
            }
            else
            {
                       ...
                       ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one =
            ImMax(1.0f, IM_FLOOR((w_inputs - (style.ItemInnerSpacing.x) *
                                                 (components - 1)) /
                                 (float)components));
...
bool ImGui::BeginListBox(const char *label, const ImVec2 &size_arg)
{
    ...
    // FIXME-OPT: We could omit the BeginGroup() if label_size.x but would need
    // to omit the EndGroup() as well.
    BeginGroup();
    if (label_size.x > 0.0f)
    {
...
bool ImGui::BeginMainMenuBar()
{
    ...
    {
        // Set window position
        // We don't attempt to calculate our height ahead, as it depends on the
        // per-viewport font size. However menu-bar will affect the minimum
        // window size so we'll get the right height.
    ...
    PushStyleVar(
        ...
        ImVec2(0, 0)); // Lift normal size constraint, however the presence of a
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        popup_pos =
            ImVec2(pos.x - 1.0f - IM_FLOOR(style.ItemSpacing.x * 0.5f),
