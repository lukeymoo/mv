...
void
Swap::init (GLFWwindow *p_GLFWwindow,
...
void
Swap::create (const vk::PhysicalDevice &p_PhysicalDevice,
              ...
              uint32_t &p_WindowHeight)
{
  ...
  // get depth format
  depthFormat = Helper::getSupportedDepthFormat ();
  ...
  for (auto &buffer : buffers)
    {
      ...
      buffer.msaaImage->create (
          p_PhysicalDevice, p_LogicalDevice, swapExtent.width, swapExtent.height, msaaInfo);
    }
  return;
}

void
Swap::cleanup (const vk::Instance &p_Instance,
               const vk::Device &p_LogicalDevice,
