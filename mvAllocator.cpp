...
vk::DescriptorSetLayout Allocator::getLayout(vk::DescriptorType p_LayoutType)
{
    if (layouts.empty())
    ...
    throw std::runtime_error("Requested layout but not found => " + descriptorTypeToString(p_LayoutType));
}

void Allocator::createLayout(vk::DescriptorType p_DescriptorType, uint32_t p_Count,
...
void Allocator::createLayout(vk::DescriptorSetLayoutCreateInfo &p_CreateInfo)
{
    ...
    {
        logger.logMessage(LogHandlerMessagePriority::eWarning, "Request to create descriptor set layout => " +
    ...
    logger.logMessage("Descriptor set layout => " + descriptorTypeToString(requestedLayoutType) + " created");
    return;
}

void Allocator::allocateSet(vk::DescriptorSetLayout &p_DescriptorLayout, vk::DescriptorSet &p_DestinationSet)
{
        ...
        case eErrorOutOfPoolMemory:
            {
                ...
                poolContainer = retryAllocateSet(p_DescriptorLayout, p_DestinationSet, poolContainer->count);
                break;
            }
        ...
        case eErrorOutOfPoolMemory:
            {
                ...
                // use new pool to allocate set
                // change passed container to new one
                p_PoolContainer = retryAllocateSet(p_DescriptorLayout, p_DestinationSet, p_PoolContainer->count);
                break;
            }
        case eErrorFragmentedPool:
            {
...
void Allocator::updateSet(vk::DescriptorBufferInfo &p_BufferDescriptor, vk::DescriptorSet &p_TargetDescriptorSet,
...
void Allocator::updateSet(vk::DescriptorImageInfo &p_ImageDescriptor, vk::DescriptorSet &p_TargetDescriptorSet,
                          uint32_t p_DestinationBinding)
{
    Container *poolContainer = &containers.at(currentPool);
...
void Allocator::loadModel(std::vector<Model> *p_Models, std::vector<std::string> *p_ModelNames, const char *p_Filename)
{
...
void Allocator::createObject(std::vector<Model> *p_Models, std::string p_ModelName)
{
    if (!p_Models)
    ...
    engine->createBuffer(vk::BufferUsageFlagBits::eUniformBuffer,
                         ...
                         &p_Models->at(modelIndex).objects->back().uniform.second, sizeof(Object::Matrix));
...
std::string Allocator::descriptorTypeToString(vk::DescriptorType p_Type)
{
    switch (p_Type)
    {
        using enum vk::DescriptorType;
